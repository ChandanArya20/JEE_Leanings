...
public class InputValidatorFilter extends HttpFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		if(name.equals("")) {
			...
			if(name.length()<3) {
				nameErrorMsg="Errro : \"Name length should not be less than 3\"";
				flag=true;
			}
		}
		if(age.equals("")) {
			...
			if(Integer.parseInt(age)<18 || Integer.parseInt(age)>30) {
				ageErrorMsg="Age should be between 18 and 30";
				flag=true;
			}
		}
		if(email.equals("")) {
			...
			if(!email.endsWith("@ineuron.ai")) {
				emailErrorMsg="Email should start with @ineuron.ai";
				flag=true;
			}
		}
		if(mob.equals("")) {
			...
			if(!mob.startsWith("91-")) {
				mobErrorMsg="Mobile No... should start with 91-";
				flag=true;
			}
		}
